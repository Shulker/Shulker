...
public class ShulkerPacketJoinGameV113R2 extends ShulkerPacketJoinGame<PacketPlayOutLogin>
{
    ...
    private static final Optional<Field> mc_dimension_field          = get_first_field_of_type(PacketPlayOutLogin.class, DimensionManager.class);
    ...
    public ShulkerPacketJoinGameV113R2()
    {
        this(new PacketPlayOutLogin());
        this.reset();
    }

    public ShulkerPacketJoinGameV113R2(int entity_id, GameMode game_mode, boolean hardcore, int dimension, int max_players, String level_type, boolean reduced_debug_info)
    {
        this(new PacketPlayOutLogin(entity_id, EnumGamemode.getById(game_mode.get_value()), hardcore, DimensionManager.a(dimension), EnumDifficulty.NORMAL, max_players, WorldType.getType(level_type), reduced_debug_info));
    }

    public ShulkerPacketJoinGameV113R2(PacketPlayOutLogin packet)
    {
        super(packet);
    }
    ...
    {
        return mc_dimension_field.map(f -> (DimensionManager) get_field_value(this.packet, f)).map(DimensionManager::getDimensionID).orElse(0);
    }

    @Override
    public void set_dimension(int dimension)
    {
        mc_dimension_field.ifPresent(f -> set_value(this.packet, f, DimensionManager.a(dimension)));
    }
